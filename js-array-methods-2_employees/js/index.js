...
function findByLastName(employees, lastName) {
  const findEmployeByLastName = employees.find((employe) => {
    return employe.lastName === lastName;
  });
  return findEmployeByLastName;
}

// Exercise 2:
// The some as above, but now you are searching by the 'id'
function findById(employees, id) {
  const findEmployeById = employees.find((employe) => {
...
function findByCitySubString(employees, string) {
  const findEmployeByCitySubString = employees.find((employe) => {
    return employe.city.startsWith(string);
  });
  return findEmployeByCitySubString;
}
...
function findByProfessionSubStringAndAge(employees, string, age) {
  const findEmployeByCitySubStringAndAge = employees.find((employe) => {
    return employe.city.startsWith(string) && employe.age > age;
  });
  return findEmployeByCitySubStringAndAge;
}
